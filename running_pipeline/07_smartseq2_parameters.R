...
############################ （一） AUROC   #######################################
setwd("./d04_BIB_parameters_cutoff001")

############################ （一） PCC   #######################################
setwd("./Benchmark_cnn_scRNA/")

############## Group ##############
# Baseline
level_order <- c('scRNA_N128L15PaP15ReLU')
# Filter Number (N)
level_order <- c('scRNA_N8L15PaP15ReLU', 'scRNA_N32L15PaP15ReLU','scRNA_N512L15PaP15ReLU')
# Filter Length (L)
level_order <- c('scRNA_N128L5PaP15ReLU', 'scRNA_N128L25PaP15ReLU')
# Pooling layer (Pa/Pm)
level_order <- c('scRNA_N128L15PmP15ReLU')
# Pooling size (P)
level_order <- c('scRNA_N128L15PaP5ReLU','scRNA_N128L15PaP25ReLU')
# Activation Function
level_order <- c('scRNA_N128L15PaP15Sigmoid','scRNA_N128L15PaP15Exp','scRNA_N128L15PaP15Tanh')
# Batch Normalization
level_order <- c('scRNA_N128L15PaP15ReLUBN','scRNA_N128L15PaP15SigmoidBN','scRNA_N128L15PaP15ExpBN','scRNA_N128L15PaP15TanhBN')
...
if(T){
  ...
  level_order <- c('scRNA_N128L15PaP15ReLU','scRNA_N128L15PmP15ReLU')
  ...
  tmp <- gsub('scRNA_N128L15','',metric1$type)
  metric1$Type <- gsub('P15ReLU','',tmp)
  dim(metric1);table(metric1$Type); head(metric1)
  
  # metric2
  # 2Filter Number (N)
  level_order <- c('scRNA_N8L15PaP15ReLU','scRNA_N32L15PaP15ReLU','scRNA_N512L15PaP15ReLU','scRNA_N128L15PaP15ReLU')
  ...
  tmp <- gsub('scRNA_','',metric2$type)
  metric2$Type <- gsub('L15PaP15ReLU','',tmp)
  dim(metric2);table(metric2$Type); head(metric2)
  
  # metric3
  # 3 Filter Length (L)
  level_order <- c( 'scRNA_N128L25PaP15ReLU','scRNA_N128L15PaP15ReLU','scRNA_N128L5PaP15ReLU')
  ...
  tmp <- gsub('scRNA_N128','',metric3$type)
  ...
  # metric4
  # Pooling size (P)
  level_order <- c('scRNA_N128L15PaP5ReLU','scRNA_N128L15PaP15ReLU','scRNA_N128L15PaP25ReLU')
  ...
  tmp <- gsub('scRNA_N128L15Pa','',metric4$type)
  metric4$Type <- gsub('ReLU','',tmp)
  dim(metric4);table(metric4$Type); head(metric4)
  
  # metric5
  # Activation Function
  level_order <- c('scRNA_N128L15PaP15Sigmoid','scRNA_N128L15PaP15Tanh','scRNA_N128L15PaP15Exp','scRNA_N128L15PaP15ReLU')
  ...
  metric5$Type <- gsub('scRNA_N128L15PaP15','',metric5$type)
  dim(metric5);table(metric5$Type); head(metric5)
  
  #  # metric6
  # Batch Normalization
  level_order <- c('scRNA_N128L15PaP15SigmoidBN',
                   'scRNA_N128L15PaP15TanhBN',
                   'scRNA_N128L15PaP15ExpBN',
                   'scRNA_N128L15PaP15ReLUBN')
  ...
  metric6$Type <- gsub('scRNA_N128L15PaP15','',metric6$type)
...
metric_total2 <- metric_total %>%
  group_by(Group) %>%
  mutate(Type = fct_reorder(Type, pcc, .fun = median, .na_rm = TRUE)) %>%
  ungroup()
...
ggplot(metric_total2, aes(x=Type, y=auroc, fill=Group)) + 
...
ggsave("./Figure3/Figure3_smartseq2_auroc_new.png", plot = P1, 
...
ggplot(metric_total2, aes(x=Type, y=pcc, fill=Group)) + 
  geom_boxplot(outlier.size = 0.2, outlier.alpha = 0.8) + theme_base() + 
  facet_grid(cols  = vars(Group), scales = 'free_x', space = 'free_x') + #分panel画
  theme(axis.text.x = element_text(angle = 45, vjust = .6, size=12),
        axis.text.y = element_text(hjust = -0.6,size=12), 
        legend.position="none") +
  ylab("PCC") + labs(x = NULL) +
  scale_fill_brewer(palette = 'Set3') + ylim(0.3,0.7)+
  stat_compare_means(aes(group = Group), method="wilcox.test",label = "p.signif", label.y = 0.7) 
#dev.off()
...
if(T){
  ...
  tmp <- gsub('scRNA_N128L15','',IC1$type)
  ...
  ################# IC2
  # Filter Number (N)
  level_order <- c('scRNA_N32L15PaP15ReLU','scRNA_N128L15PaP15ReLU', 'scRNA_N512L15PaP15ReLU','scRNA_N8L15PaP15ReLU')
  ...
  # IC$IC <- ifelse(IC$IC > 8, 8, IC$IC)
  # IC$IC <- ifelse(IC$IC < 6, 6, IC$IC)
  tmp <- gsub('scRNA_','',IC2$type)
  IC2$Type <- gsub('L15PaP15ReLU','',tmp)
  dim(IC2); head(IC2)
  
  ################# IC3
  # Filter Length (L)
  level_order <- c('scRNA_N128L5PaP15ReLU', 'scRNA_N128L25PaP15ReLU', 'scRNA_N128L15PaP15ReLU')
  ...
  # IC$IC <- ifelse(IC$IC > 8, 8, IC$IC)
  # IC$IC <- ifelse(IC$IC < 6, 6, IC$IC)
  tmp <- gsub('scRNA_N128L15Pa','',IC4$type)
  IC4$Type <- gsub('ReLU','',tmp)
  dim(IC4); head(IC4)
  
  ################# IC5
  # Activation Function
  level_order <- c('scRNA_N128L15PaP15ReLU', 'scRNA_N128L15PaP15Sigmoid','scRNA_N128L15PaP15Tanh','scRNA_N128L15PaP15Exp')
  ...
  # IC$IC <- ifelse(IC$IC < 6, 6, IC$IC)
  IC5$Group <- 'Activation Function'
  IC5$Type <- gsub('scRNA_N128L15PaP15','',IC5$type)
  dim(IC5); head(IC5)
  
  ################# IC6
  # Batch Normalization
  level_order <- c('scRNA_N128L15PaP15ReLUBN',
                   'scRNA_N128L15PaP15SigmoidBN',
                   'scRNA_N128L15PaP15TanhBN',
                   'scRNA_N128L15PaP15ExpBN')
  ...
  # IC$IC <- ifelse(IC$IC > 8, 8, IC$IC)
  # IC$IC <- ifelse(IC$IC < 6, 6, IC$IC)
  IC6$Type <- gsub('scRNA_N128L15PaP15','',IC6$type)
  dim(IC6); head(IC6)
}
...
IC_total2 <- IC_total %>%
  group_by(Group) %>%
  mutate(Type = fct_reorder(Type, IC, .fun = median, .na_rm = TRUE)) %>%
  ungroup()
...
ggsave("./Figure3_AUROC_smartseq2_IC_new.png", plot = P2, 
       width = 10.5, height = 8, units = "cm", dpi = 300,
       device = "png", bg = "white") 
